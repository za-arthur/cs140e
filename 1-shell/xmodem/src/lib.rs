...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, msg: &'static str) -> io::Result<u8> {
        match self.read_byte(byte != CAN) {
            Ok(read_byte) if read_byte == byte => return Ok(byte),
            Ok(_) => {
                self.write_byte(CAN)?;
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        match self.read_byte(byte != CAN) {
            Ok(read_byte) if read_byte == byte => return Ok(byte),
            Ok(_) => return Err(io::Error::new(io::ErrorKind::InvalidData, expected)),
            Err(e) => return Err(e)
        }
    }
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        match self.read_byte(true)? {
            ...
            SOH => {
                let _packet = self.packet;
                let mut sum: u8 = 0;

                self.expect_byte_or_cancel(_packet, "got wrong packet number")?;
                self.expect_byte_or_cancel(255 - _packet,
                                           "got wrong 1s complement")?;
                ...
                match self.read_byte(false) {
                    Ok(s) if s == sum => {
                        self.write_byte(ACK)?;
                        (self.progress)(Progress::Packet(_packet));
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        match buf.len() {
            ...
            _ => {
                ...
                self.write_byte(SOH)?;
                self.write_byte(_packet)?;
                self.write_byte(255 - _packet)?;
                ...
                match self.read_byte(true)? {
                    NAK => return Err(io::Error::new(io::ErrorKind::Interrupted,
                                                     "checksums differ")),
                    ACK => {
                        (self.progress)(Progress::Packet(_packet));
                        self.packet = self.packet.wrapping_add(1);
